...
- **在「群配置」弹窗里新增群号后，该表的问答对会被清空**：弹窗只负责「服务哪些群号 + 名称」，
  却把「当前选中的表」的问答对一并提交，而后端按整体替换写入。当选中表不是被编辑的那张表时
  （默认选中的全局表往往没有问答对），被编辑表的问答对就被写空，只留下答案池。现在：
  - 弹窗保存不再提交 `entries`；后端在缺省该项时**原样保留问答对**，只更新群号与名称；
  - 后端新增数据安全网：**用空列表覆盖非空表会被拒绝（400）**并说明原因，
    只有问答表编辑页（显式带 `allow_empty`）才能把一张表清空。
- 事故影响范围仅限「用弹窗改群号/名称」这一条路径；问答表编辑页的保存不受影响。

### 恢复

- 若已经丢过数据：用事故发生前的 `qa_tables.json` 备份与当前文件的群号合并即可还原，
  参见 README「数据存放」里的备份与回滚说明。

### 测试

- 新增 `tests/test_qa_group_config_save.py`（25 项）：覆盖省略 entries 的群配置保存、
  空列表覆盖拦截、显式清空、带 entries 的整体替换、新建群配置与非法 entries 类型。
- 页面冒烟新增 2 项断言：群配置保存不得提交 `entries`，编辑页保存必须提交 `entries` 且带 `allow_empty`；
  这两项断言在修复前的实现下会失败，可作为真实回归保护。
